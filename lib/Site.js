...
Site.prototype.createPageData = function (config) {
  ...
  return new Page({
    ...
    asset: {
      ...
      typeahead: path.relative(path.dirname(resultPath),
                               path.join(this.siteAssetsDestPath, 'js', 'typeahead.bundle.min.js')),
      jquery: path.relative(path.dirname(resultPath),
                            path.join(this.siteAssetsDestPath, 'js', 'jquery-3.3.1.min.js')),
      search: path.relative(path.dirname(resultPath),
                            path.join(this.siteAssetsDestPath, 'js', 'search.js'))
    },
  });
};
